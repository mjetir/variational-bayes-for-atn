...
function initialize_OLS_options(fw_init,training_data;kwargs...)
    ...
    nPar = 2*fw_init.nLinks

    var_ini = vcat(fw_init.fr_half_sat[fw_init.I_ind],
    fw_init.fr_shape[fw_init.I_ind])

    param_min::Vector{Float64} = zeros(nPar)
    # half-saturation constants
    param_min[1:fw_init.nLinks] .= 0.05
    # the type of functional response
    param_min[fw_init.nLinks+1:2*fw_init.nLinks] .= 0.3
...
function initialize_bayes_options(fw_init,rho,training_data;kwargs...)
    ...
    nPar = fw_init.nSpec + 2*fw_init.nLinks

    par_means_ini = vcat(fw_init.fr_half_sat[fw_init.I_ind],
        fw_init.fr_shape[fw_init.I_ind],
        fw_init.std)
    ...
    # half-saturation constants
    param_min[1:fw_init.nLinks] .= 0.05
    # the types of functional response
    param_min[fw_init.nLinks+1:2*fw_init.nLinks] .= 0.3
    # keep stds strictly positive but otherwise, no restrictions from below:
    param_min[2*fw_init.nLinks+1:2*fw_init.nLinks+fw_init.nSpec] .= 0.0000000001
    ...
    # restrict std based on training data:
    for i=1:fw_init.nSpec
            sorted_training_data = sort(training_data[i,:])
            param_max[2*fw_init.nLinks+i] = maximum([Statistics.stdm(training_data[i,:],sorted_training_data[5]),
                                                      Statistics.stdm(training_data[i,:],sorted_training_data[end-4])]) 
    end
#    println("Upper bounds of std:")
#    println(param_max[2*fw_init.nLinks+1:2*fw_init.nLinks+fw_init.nSpec])
